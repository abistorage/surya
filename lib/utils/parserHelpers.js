...
var parserHelpers = {
  ...
  isAContractTypecast: function isAContractTypecast(node, contractNames) {
    var expr = node.expression.expression; // @TODO: replace lowercase for better filtering
  ...
  isSpecialVariable: function isSpecialVariable(node) {
    ...
    if (node.hasOwnProperty('type') && node.type === 'Identifier' && node.name === 'now') {
      return true; // any block.<x> special variable
      ...
      return true; // any msg.<x> special variable
      ...
      return true; // any tx.<x> special variable
