...
function graph(files) {
    ...
    var _loop2 = function _loop2() {
      ...
      parser.visit(ast, {
        ContractDefinition: function ContractDefinition(node) {
          ...
          if (!(cluster = digraph.getCluster("\"cluster".concat(contractName, "\"")))) {
            cluster = digraph.addCluster("\"cluster".concat(contractName, "\""));
  ...
  var _loop = function _loop() {
    ...
    function nodeName(functionName, contractName) {
      if (dependencies.hasOwnProperty(contractName)) {
        ...
        try {
          for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
            ...
            var name = "".concat(dep, ".").concat(functionName);
      ...
      return "".concat(contractName, ".").concat(functionName);
    }
    ...
    parser.visit(ast, {
      ContractDefinition: function ContractDefinition(node) {
        contractName = node.name;
        cluster = digraph.getCluster("\"cluster".concat(contractName, "\""));
      },
      'ContractDefinition:exit': function ContractDefinitionExit(node) {
        contractName = '0_global';
      },
    ...
    parser.visit(ast, {
      ...
      ModifierInvocation: function ModifierInvocation(node) {
        if (options.enableModifierEdges && callingScope) {
          digraph.addEdge(callingScope, nodeName(node.name, contractName), {
            color: 'yellow'
          });
        }
      },
      FunctionCall: function FunctionCall(node) {
        ...
        if (!(externalCluster = digraph.getCluster("\"cluster".concat(localContractName, "\"")))) {
          externalCluster = digraph.addCluster("\"cluster".concat(localContractName, "\""));
